...
//! 
//! LIMITATIONS
//! TOKENSET CAN ONLY CONTAIN ONE OF FIRST 128 SYNTAX KINDS
//! Parser::at_ts(..) only works for non-composite tokens!
//! 
...
impl<'t> Parser<'t> {
    ...
    pub(crate) fn nth_at(&self, n: usize, kind: SyntaxKind) -> bool {
        match kind {
            T![*=] => self.at_composite2(n, T![*], T![=]), // attribute selector substring operator
            T![|=] => self.at_composite2(n, T![|], T![=]), // attribute selector dashmatch operator
            SyntaxKind::OPERATOR_SUFFIX => self.at_composite2(n, SyntaxKind::DOLLAR, T![=]), // attribute selector suffix operator
            T![^=] => self.at_composite2(n, T![^], T![=]), // attribute selector prefix operator
            T![~=] => self.at_composite2(n, T![~], T![=]), // attribute selector includes operator
    ...
    fn at_composite2(&self, n: usize, k1: SyntaxKind, k2: SyntaxKind) -> bool {
        self.inp.kind(self.pos + n) == k1
            ...
            && !self.inp.has_whitespace_after(self.pos + n)
    }
    ...
    fn at_composite3(&self, n: usize, k1: SyntaxKind, k2: SyntaxKind, k3: SyntaxKind) -> bool {
        self.inp.kind(self.pos + n) == k1
            ...
            && !self.inp.has_whitespace_after(self.pos + n)
            && !self.inp.has_whitespace_after(self.pos + n + 1)
    }

    /// Returns if there was whitespace at the current position. 
    #[inline]
    ...
    /// Checks if there is a parse error after the start
    /// of the `n`th latest marker that has not been completed/abandoned
    /// yet.
    pub(crate) fn did_err_since_last_unfinished(&self) -> bool {
        for e in self.events.iter().rev() {
            match e {
                Event::Error { .. } => return true,
                Event::Start { kind, ..} => {
...
impl Marker {
    ...
    /// Experimental
    /// Abandons the syntax tree node and rolls back
    /// the parsing of all its children.
    pub(crate) fn rollback(mut self, p: &mut Parser<'_>) {
        self.bomb.defuse();
        let idx = self.pos as usize;
        while idx < p.events.len() - 1  {
            match p.events.pop() {
                Some(Event::Token { n_raw_tokens , ..}) => {
                    p.pos -= n_raw_tokens as usize;
                },
                Some(_) => {}
                None => {unreachable!()}
            }
        }
        assert!(idx == p.events.len() - 1);
        match p.events.pop() {
            Some(Event::Start { kind: TOMBSTONE, forward_parent: None }) => (),
            _ => unreachable!(),
        }
